...
sample_opt.w_handle = @one_w; % Minimize l2-coherence
sample_opt.w_correction_handle = @one_correction_w; % Minimize l2-coherence
sample_opt.prop_handle = @orth_prop; % input proposals are from orthogonality measure
sample_opt.max_sample_percent = .3; % Maximum proportion of samples to add on each iteration
...
for kk = 1:5 
    ...
    start_sample = 100;
    sample_opt.initial_size = 4000; % Occasionally want initial_size to differ from sample_opt.sr
    sample_opt.min_sample_percent = 0.1; % Percent new samples to generate
    full_sample = orth_sampler(sample_opt, eval_opt); % Initialization of Samples
    sample.w = ones(start_sample,1);
    sample.rv = full_sample.rv(1:start_sample,:);
    sample.rhs = full_sample.rhs(1:start_sample,:);
    sample.n_samps = start_sample;
    sample.lhs = basis_eval(basis,sample.rv,eval_opt);
    sample.wlhs =  sample.lhs;
...
% ---- %
    k = 0;
    str = strcat('duffing_ex_no_sa_',num2str(kk));
    while(k < max_iter && sample.n_samps < sample_opt.max_samps) % Several exit criteria    
     %   sample_opt.old_basis = basis; % Old basis needed to correct samples
        ...
        start_sample = ceil(start_sample*(1+sample_opt.min_sample_percent)); % Occasionally want initial_size to differ from sample_opt.sr
        sample.rv = full_sample.rv(1:start_sample,:);
        sample.rhs = full_sample.rhs(1:start_sample,:);
        sample.n_samps = start_sample;
        sample.lhs = basis_eval(basis,sample.rv,eval_opt);
